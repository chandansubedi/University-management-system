...
class Command(BaseCommand):
    help = 'Reset password for a student account'

    def add_arguments(self, parser):
        parser.add_argument('username', type=str, help='Username or email of the student')
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if user.user_type != 3:
                raise CommandError(f'User "{username}" is not a student (user_type: {user.user_type})')
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully reset password for student "{user.username}" (email: {user.email})')
            )
